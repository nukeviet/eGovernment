...
list($personid, $organid, $old_weight) = $db->query("SELECT personid , organid, weight FROM " . $table . " WHERE personid=" . intval($personid) . "")->fetch(3);
if ($personid > 0) {
    $personid_swap = $db->query("SELECT personid FROM " . $table . " WHERE organid = " . intval($organid) . " AND weight = " . intval($new_weight))->fetchColumn();

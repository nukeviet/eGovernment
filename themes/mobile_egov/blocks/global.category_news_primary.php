...
if (!nv_function_exists('nv_block_block_news_category_primary')) {
    ...
    function nv_block_config_block_news_category_primary($module, $data_block, $lang_block)
    {
        ...
        $html .= '	<div class="col-sm-9"><select name="config_module_name" id="config_module_name" class="form-control">';
        ...
        $html .= '<div class="col-sm-18" id="div-catid"></div>';
        $html .= '</div>';
        ...
        $html .= '<div class="col-sm-5"><input type="text" class="form-control" name="config_content_length" value="' . $data_block['content_length'] . '"/></div>';
        $html .= '</div>';
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        if ($module == $module_name) {
            ...
            $sql = 'SELECT catid, parentid, title, alias, viewcat, subcatid, numlinks, description, status, keywords, groups_view FROM ' . NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'] . '_cat WHERE status=1 OR status=2 ORDER BY sort ASC';
